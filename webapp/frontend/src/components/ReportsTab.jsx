...
export default function ReportsTab({ logs = [], childName = "Child", caregiverName = "User", caregiverRole = "Parent/Caregiver" }) {
    ...
    const handleDownloadPDF = () => {
        ...
        doc.text(`${childName}'s Activity Logs`, 14, 20);
        ...
        let currentY = 33;
        const preparedByText = caregiverName ? `${caregiverName} - ${caregiverRole}` : `admin - ${caregiverRole}`;
        doc.text(`Prepared By: ${preparedByText}`, 14, currentY);
        currentY += 5;

        doc.text(`Report Period: ${dateRange.toUpperCase()}`, 14, currentY);
        currentY += 5;
        doc.text(`Total Observations: ${filteredLogs.length}`, 14, currentY);
        currentY += 7;
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden">
                ...
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white">{childName}'s Activity Logs</h2>
                    ...
                    <div className="flex gap-2 w-full md:w-auto">
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-bold transition-all shadow-lg ${filteredLogs.length === 0
                                ? 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none dark:bg-slate-800 dark:text-slate-600 border border-slate-300 dark:border-slate-700'
                ...
                <div className="px-6 py-4 bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-3 w-full">
                        ...
                        <div className="flex gap-1 shrink-0">
                            {TYPE_FILTERS.map(f => (
                                <button
                                    ...
                                    className={`px-2 py-1.5 rounded-lg text-xs font-bold transition-all ${typeFilter === f.id ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-500'}`}
                                >
                                    {f.label}
                                </button>
                            ))}
                        ...
                        <div className="flex gap-1 border-l border-slate-200 dark:border-slate-700 pl-3 shrink-0">
                            {EMOTION_FILTERS.map(f => (
                                <button
                                    key={f.id}
                                    onClick={() => setEmotionFilter(emotionFilter === f.id ? 'all' : f.id)}
                                    className={`px-2 py-1.5 rounded-lg text-xs font-bold transition-all ${emotionFilter === f.id ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-500'}`}
                                >
                                    <span className="hidden xl:inline">{f.label}</span>
                                    <span className="xl:hidden">{f.label.charAt(0)}</span>
                                </button>
                            ))}
                        </div>

                        {/* Date Range & Reset Container */}
                        <div className="flex gap-1 items-center border-l border-slate-200 dark:border-slate-700 pl-3 shrink-0 relative">
                            {DATE_RANGES.map(f => (
                                <button
                                    key={f.id}
                                    onClick={() => setDateRange(dateRange === f.id ? 'all' : f.id)}
                                    className={`px-2 py-1.5 rounded-lg text-xs font-bold transition-all ${dateRange === f.id ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-500'}`}
                                >
                                    {f.label}
                                </button>
                            ))}
                            ...
                            {hasActiveFilters && (
                                <button onClick={clearAllFilters} className="text-xs font-bold text-red-500 flex items-center gap-1 hover:bg-red-100 dark:hover:bg-red-900/40 shrink-0 whitespace-nowrap ml-1 bg-red-50 dark:bg-red-900/20 px-2.5 py-1.5 rounded-lg transition-colors">
                                    <X size={14} /> Clear
                                </button>
                            )}

                            {/* Custom Date Inputs (Absolute Popover) */}
                            <AnimatePresence>
                                {dateRange === 'custom' && (
                                    <motion.div initial={{ opacity: 0, scale: 0.95, y: -5 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -5 }} className="absolute right-0 top-full mt-2 p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl flex gap-2 items-center shrink-0 z-50 origin-top-right">
                                        <div className="flex flex-col gap-1">
                                            <span className="text-[10px] uppercase font-bold text-slate-400">Start</span>
                                            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="px-2 py-1.5 text-xs rounded border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 outline-none focus:ring-2 focus:ring-indigo-500/50" />
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            <span className="text-[10px] uppercase font-bold text-slate-400">End</span>
                                            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="px-2 py-1.5 text-xs rounded border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 outline-none focus:ring-2 focus:ring-indigo-500/50" />
                                        </div>
                                    </motion.div>
                                )}
                            </AnimatePresence>
